...
def super_add(p):
    if isinstance(release_parser.generatedStructs.get(p[2])[1], Map):
        temp_map = map_no_elements(release_parser.generatedStructs.get(p[2])[1], p[5])
        release_parser.generatedStructs[str(p[2])] = ("Map", temp_map)
        print("Added " + str(p[5]) + " to Map " + str(p[2]))
        release_parser.messages.append("Added " + str(p[5]) + " to Map " + str(p[2]))
    elif isinstance(release_parser.generatedStructs.get(p[2])[1], BinarySearchTree):
        temp_tree = tree_no_elements(release_parser.generatedStructs.get(p[2])[1], p[5])
        release_parser.generatedStructs[str(p[2])] = ("Tree", temp_tree)
        print("Added " + str(p[5]) + " to Tree " + str(p[2]))
        release_parser.messages.append("Added " + str(p[5]) + " to Tree " + str(p[2]))
    elif isinstance(release_parser.generatedStructs.get(p[2])[1], CircularDoublyLinkedList):
        temp_cdll = cdll_no_elements(release_parser.generatedStructs.get(p[2])[1], p[5])
        release_parser.generatedStructs[str(p[2])] = ("CDLL", temp_cdll)
        print("Added " + str(p[5]) + " to CDLL " + str(p[2]))
        release_parser.messages.append("Added " + str(p[5]) + " to CDLL " + str(p[2]))
    elif isinstance(release_parser.generatedStructs.get(p[2])[1], ArraySet):
        temp_set = set_no_elements(release_parser.generatedStructs.get(p[2])[1], p[5])
        release_parser.generatedStructs[str(p[2])] = ("Set", temp_set)
        print("Added " + str(p[5]) + " to ArraySet " + str(p[2]))
        release_parser.messages.append("Added " + str(p[5]) + " to ArraySet " + str(p[2]))
    elif isinstance(release_parser.generatedStructs.get(p[2])[1], DynamicBag):
        temp_bag = bag_no_elements(release_parser.generatedStructs.get(p[2])[1], p[5])
        release_parser.generatedStructs[str(p[2])] = ("Bag", temp_bag)
        print("Added " + str(p[5]) + " to Bag " + str(p[2]))
        release_parser.messages.append("Added " + str(p[5]) + " to Bag " + str(p[2]))
    elif isinstance(release_parser.generatedStructs.get(p[2])[1], SinglyLinkedList):
        temp_list = linkedlist_no_elements(release_parser.generatedStructs.get(p[2])[1], p[5])
        release_parser.generatedStructs[str(p[2])] = ("SinglyLinkedList", temp_list)
        print("Added " + str(p[5]) + " to SLL " + str(p[2]))
        release_parser.messages.append("Added " + str(p[5]) + " to SLL " + str(p[2]))
    elif isinstance(release_parser.generatedStructs.get(p[2])[1], HashTableOA):
        temp_table = hashtable_no_elements(release_parser.generatedStructs.get(p[2])[1], p[5])
        release_parser.generatedStructs[str(p[2])] = ("HSOA", temp_table)
        print("Added " + str(p[5]) + " to HSOA " + str(p[2]))
        release_parser.messages.append("Added " + str(p[5]) + " to HSOA " + str(p[2]))
    elif isinstance(release_parser.generatedStructs.get(p[2])[1], HashTableSC):
        temp_table = hashtable_no_elements(release_parser.generatedStructs.get(p[2])[1], p[5])
        release_parser.generatedStructs[str(p[2])] = ("HSSC", temp_table)
        print("Added " + str(p[5]) + " to HSSC " + str(p[2]))
        release_parser.messages.append("Added " + str(p[5]) + " to HSSC " + str(p[2]))
    elif isinstance(release_parser.generatedStructs.get(p[2])[1], SortedArrayList):
        temp_sl = sortedarraylist_no_elements(release_parser.generatedStructs.get(p[2])[1], p[5])
        release_parser.generatedStructs[str(p[2])] = ("SortedList", temp_sl)
        print("Added " + str(p[5]) + " to SortedList " + str(p[2]))
        release_parser.messages.append("Added " + str(p[5]) + " to SortedList " + str(p[2]))
    elif isinstance(release_parser.generatedStructs.get(p[2])[1], Stack):
        temp_stack = stack_no_elements(release_parser.generatedStructs.get(p[2])[1], p[5])
        release_parser.generatedStructs[str(p[2])] = ("Stack", temp_stack)
        print("Added " + str(p[5]) + " to Stack " + str(p[2]))
        release_parser.messages.append("Added " + str(p[5]) + " to Stack " + str(p[2]))
    elif isinstance(release_parser.generatedStructs.get(p[2])[1], DoublyLinkedQueue):
        temp_queue = queue_no_elements(release_parser.generatedStructs.get(p[2])[1], p[5])
        release_parser.generatedStructs[str(p[2])] = ("Queue", temp_queue)
        print("Added " + str(p[5]) + " to Queue " + str(p[2]))
        release_parser.messages.append("Added " + str(p[5]) + " to Queue " + str(p[2]))
